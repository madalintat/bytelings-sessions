...
If you're someone who *kind of* knows Python (or wants to), but still freezes when asked to write something from scratch — this is for you. bytelings is a curriculum that meets you exactly where you are and walks you, day by day, into real fluency.
...
Now save a file. Watch the tests turn green. **That's the loop.** ✨
...
🔍 &nbsp; **Read** &nbsp; — a short, story-styled page. Not a textbook chapter. Just enough to *feel* the idea.

🔧 &nbsp; **Drill** &nbsp; — a broken file with a couple of `# TODO` lines. Fix them. Tests turn green when you nail it.

🪜 &nbsp; **Guided** &nbsp; — here's a function signature and a docstring. You write the body.

🦾 &nbsp; **Solo** &nbsp; — same idea, no scaffold. Hidden tests. You write it cold, like real work.

🎯 &nbsp; **Apply** &nbsp; — a tiny project chunk that uses what you just wrote. The concept lands somewhere real.
...
This one matters. Same-shape lessons make brains autopilot. So bytelings rotates across **8 different storytelling styles** — and you'll never see the same one two days in a row:

📖 **Story** — *"It's Monday, 9 AM. You just got hired. Here's the repo..."*
🩹 **Pain-point** — show ugly buggy slow code first, then the fix
⚖️ **Compare two ways** — two snippets side by side, you pick the right one
🚶 **Code tour** — read a real piece of code line by line
🔮 **Trace** — predict what each line prints *before* you run it
🛠️ **Build it yourself** — *"Pretend Python doesn't have this. How would you build it?"*
🌉 **Metaphor** — concepts mapped onto things from real life
🕵️ **Detective** — there's a bug, the concept is the only fix

Your brain stays alert. The fluency builds.

---

## 🗺️ What you'll learn — the 135-day map
...
🛠️ **After every phase, a 3-day project** that combines everything you just learned:
contacts manager → async website snapshotter → parallel log analyzer → tiny in-memory database → maze pathfinder. Phase 6 ends in an **8-day capstone** where you build, test, and ship a CLI tool of your own design.
...
| | |
|---|---|
| `bytelings` | Start the watcher. Save → green panel → next rung. ✨ |
...
bytelings is the curriculum I wished existed. Now it does. 🎉
...
Want to help build a track? See contributing below. 👇
